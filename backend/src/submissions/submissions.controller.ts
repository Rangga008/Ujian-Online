...
export class SubmissionsController {
  ...
  startExam(@Param('examId') examId: string, @Request() req) {
    return this.submissionsService.startExam(req.user.userId, +examId);
  }
  ...
  @Roles(UserRole.STUDENT)
  @UseGuards(RolesGuard)
  submitExam(@Param('id') id: string, @Request() req) {
    return this.submissionsService.submitExam(+id, req.user.userId);
  }
