...
export class SubmissionsService {
	...
	async findByExam(examId: number): Promise<Submission[]> {
		...
		console.log(
			`DEBUG - findByExam(${examId}): Found ${submissions.length} submissions`
		);
	...
	async toggleAnswerCorrect(
		...
		const answer = await this.answersRepository.findOne({
			where: { id: answerId, submissionId },
			relations: ["question"],
		});
		...
		if (answer.question?.type === QuestionType.ESSAY) {
			// For essay questions, keep the existing points (admin set)
			// Don't change points when toggling correct
		} else {
			// For auto-graded questions, set points based on isCorrect
			answer.points = isCorrect ? answer.question?.points || 0 : 0;
		}

		await this.answersRepository.save(answer);

		// Reload and return full submission with updated data
